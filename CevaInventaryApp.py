...
class DatabaseApp:
    def __init__(self, root):
        ...
        self.tree = ttk.Treeview(self.root, style="Treeview", height=900)
        self.tree.pack()
    ...
    def change_table(self, table_name):
        ...
        if table_name in self.column_configurations:
            ...
            for col in column_names:
                self.tree.column(col, width=150, anchor="center")  # adjust the width to suit your needs
                self.column_configurations[table_name][col] = 150  # Store the width
    ...
    def update_row(self, new_values):
        if self.current_row:
            cursor = self.connection.cursor()
            # Assuming three columns: id, name, value
            cursor.execute(f"UPDATE {self.current_table} SET name = '{new_values[0]}', value = '{new_values[1]}' WHERE id = {self.current_row[0]}")
            self.connection.commit()
            self.change_table(self.current_table)  # Refresh table
    ...
    def update_row_dialog(self):
        ...
        entries = []
        for i, value in enumerate(self.current_row):
            Label(update_dialog, text=f"Field {i}").grid(row=i, column=0)
        ...
        Button(update_dialog, text="Update", command=lambda: self.update_row([e.get() for e in entries])).grid(row=len(self.current_row), column=0, columnspan=2)
